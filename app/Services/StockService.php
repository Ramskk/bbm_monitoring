...
class StockService
{
     ...
     * @return array
     */
    public function kurangiStok(int $bbmId, float $jumlahLiter, string $referensiNo, string $referensiType, int $referensiId, string $keterangan): array
    {
        $result = [];
        ...
        try {
            ...
            if (!$bbm) {
                throw new QueryException("BBM dengan id {$bbmId} tidak ditemukan.");
            }

            $stok = $bbm->stok;
            if (!$stok) {
                throw new QueryException("Stok untuk BBM {$bbm->nama} tidak ditemukan.");
            }
     ...
     * @param int $bbmId
     * @param float $jumlahLiter
     * @param float $hargaPerLiter
     * @param string $referensiNo
     * @param string $referensiType
     * @param int $referensiId
     * @return array
     */
    public function tambahStok(int $bbmId, float $jumlahLiter, float $hargaPerLiter, string $referensiNo, string $referensiType, int $referensiId): array
    {
        $result = [];
     ...
     *
     * @param int $mutasiStokId
     * @return array
     */
    public function rollbackKeluar(int $mutasiStokId): array
    {
        $result = [];
        ...
        try {
            ...
            if (!$mutasiStok) {
                throw new QueryException("Mutasi stok dengan id {$mutasiStokId} tidak ditemukan.");
            }

            $stok = $mutasiStok->stok;
            if (!$stok) {
                throw new QueryException("Stok untuk mutasi stok {$mutasiStokId} tidak ditemukan.");
            }
     ...
     * @return bool
     */
    public function cekStokCukup(int $bbmId, float $jumlahLiter): bool
    {
