                ...
                <div class="bg-white rounded-lg shadow border border-gray-200">
                            ...
                            @forelse($vendorList as $v)
                                <tr class="border-t border-gray-100 hover:bg-gray-50">
                                    ...
                                    <td class="text-center px-4 py-3">
                                        <span class="px-2 py-1 rounded-full text-xs {{ $v->status === 'aktif' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700' }}">
                                            {{ $v->status_badge }}
                                        </span>
                                    </td>
                                    <td class="text-center px-4 py-3">
                                        <a href="{{ route('vendor.show', $v) }}" class="text-blue-600 hover:underline mr-2">Detail</a>
                                        @if(auth()->check() && auth()->user()->hasAnyRole(['admin', 'super_admin', 'admin_pengadaan']))
                                            <a href="{{ route('vendor.edit', $v) }}" class="text-yellow-600 hover:underline mr-2">Edit</a>
                                        @endif
                                        @if(auth()->check() && auth()->user()->hasAnyRole(['admin', 'super_admin', 'admin_pengadaan']) && $v->po->count() === 0)
                                            <a href="{{ route('vendor.destroy', $v) }}" class="text-red-600 hover:underline" onclick="return confirm('Akan menghapus vendor ini. Apakah Anda yakin?')">Hapus</a>
                                        @endif
                                    </td>
                                </tr>
                            @empty
                                <tr><td colspan="8" class="px-4 py-8 text-center text-gray-400">Belum ada vendor</td></tr>
                            @endforelse
                        </tbody>
                    </table>
                    ...
                    @if($vendorList->hasPages())
                        <div class="px-4 py-3 border-t border-gray-200 flex justify-between items-center">
                            <span class="text-sm text-gray-600">Menampilkan {{ $vendorList->from() }} - {{ $vendorList->to() }} dari {{ $vendorList->total() }} vendor</span>
